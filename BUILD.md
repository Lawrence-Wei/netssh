...
```bash
sed -i 's/^version = ".*"/version = "1.1.14"/' src-tauri/Cargo.toml
```
...
├── msi\   netssh_1.1.14_x64_en-US.msi      # MSI 安装包
├── nsis\  netssh_1.1.14_x64-setup.exe      # NSIS 安装程序（推荐）
└── ...
```
...
git commit -m "bump version to 1.1.14"
...
if (-not $newVersion) {
    Write-Host "Usage: .\release.ps1 1.1.14"
    exit 1
}
